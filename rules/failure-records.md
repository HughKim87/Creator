...
- Reconcile failure knowledge in the same task after successful verification and before reporting completion: unresolved failures remain in `SESSION_HANDOFF.md`, recurrences update their existing case, and newly confirmed causes create a case. Required case, index, projection, and lifecycle reconciliation are completion evidence, not optional feature expansion.
...
- When a projected failure Markdown file changes, treat its existing structured projection as stale immediately and complete the approved lifecycle replacement before reporting the recording task complete. Staleness is a reconciliation trigger, never a reason to skip or defer the canonical Markdown update.
