...
- When independent subagent validation is part of the gate, finish the scoped implementation and local verification first, then create exactly one fresh read-only validator for that validation task without inherited conversation context. Provide only the current requirements and scope, final artifacts or diff, completed test evidence, declared unresolved risks, and pass/fail contract; do not provide past findings, fix or retry history, or a desired verdict.
- Do not reuse a validator after changing the validated result. A materially changed result is a new validation task and, if validation is still authorized and required, must be reviewed by a new blind validator. Do not add multiple validators to the same validation task.
