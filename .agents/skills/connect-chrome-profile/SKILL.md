...
3. 실행 전 확장 인스턴스와 열린 탭 기준선을 보관한다. 실행 후 2초는 최초 재조회 시점으로만 사용하고, 그 시점에 후보가 없어도 실패로 판정하지 않는다. 이후 최대 15초 동안 1초 간격으로 브라우저 목록과 열린 탭을 재조회해 확장 연결 지연을 흡수한다. 새 후보가 나타나면 즉시 polling을 종료한다.
4. 실행 전후 차이에서 새로 나타난 브라우저 인스턴스 또는 새 `about:blank` 탭이 하나로 식별되면 그 인스턴스를 선택한다. 같은 탭 목록에 늦게 나타난 후보도 기준선과 대조해 동일하게 처리한다.
5. 위 bounded polling이 끝난 뒤에도 대상 프로필의 새 연결이 나타나지 않으면 그때 대상 프로필 통신 실패로 보고, 아직 수행하지 않은 경우에만 2단계의 공식 진단을 한 번 실행한다.
...
- 첫 통신 실패는 Chrome 문제 해결 문서에 따라 2초 후 같은 가벼운 호출을 한 번만 재시도한다. 프로필 창 실행 뒤 새 탭 탐색은 별도 절차로, 2초 단일 확인이 아니라 위 bounded polling을 적용한다.
