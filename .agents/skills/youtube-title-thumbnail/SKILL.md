...
5. 사용자가 승인하거나 썸네일 선택 권한을 명시적으로 위임한 경우에만 `approved`로 바꾼다.
...
사용자가 정확한 문구를 승인하기 전에는 이미지 생성 도구를 호출하지 않는다. “끝까지 진행”, 일괄 진행이나 권장안 임의 선택은 썸네일 문구 승인 생략으로 해석하지 않는다. 사용자가 썸네일 승인 생략을 정확히 명시한 경우에만 `delegated_by_user`로 기록한다.
...
최종 이미지와 320×180 미리보기를 사용자에게 보여준다. 사용자가 승인하거나 최종 시각 선택을 명시적으로 위임하기 전에는 `visual.status`를 `approved`로 바꾸거나 단계를 완료하지 않는다.
