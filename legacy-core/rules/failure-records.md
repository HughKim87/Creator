...
Create or update `core/failures/*.md` only when the cause and resolution are verified and at least one is true:
...
- Search `core/failures/README.md` before creating a case. Merge a recurrence into the same root-cause owner only when it adds useful prevention or material recurrence evidence.
...
- When a failure Markdown changes, validate that document and its links. Update `core/failures/README.md` only when a case is added, removed, or renamed.
