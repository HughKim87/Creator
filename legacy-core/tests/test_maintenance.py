...
from file_data.maintenance import MaintenanceService
...
class MaintenanceServiceTests(unittest.TestCase):
    ...
    def _fixture(self, raw_root: str) -> tuple[MaintenanceService, dict, dict]:
        ...
        (root / "core" / "src" / "file_data").mkdir(parents=True)
        (root / "core" / "schemas").mkdir()
        (root / "extension" / "src").mkdir(parents=True)
        (root / "extension" / "schemas").mkdir()
        (root / "README.md").write_text("# 테스트\n\n[현재](SESSION_HANDOFF.md)\n", encoding="utf-8")
        (root / "SESSION_HANDOFF.md").write_text("# 현재 상태\n", encoding="utf-8")
        (root / "core" / "docs" / "source.md").write_text("# 출처\n\nversion one\n", encoding="utf-8")
        (root / "core" / "src" / "file_data" / "neutral.py").write_text(
            "VALUE = 1\n", encoding="utf-8"
        )
        (root / "core" / "schemas" / "neutral.json").write_text(
            "{}\n", encoding="utf-8"
        )
        ...
        source = knowledge.create_source(
            source_kind="local_document", locator="core/docs/source.md", evidence_role="primary",
            record_id=str(uuid4())
        )
    ...
    def _write_knowledge_replacement(
        ...
        *,
        ...
        target: str = "core/docs/canonical.md",
...
{{
  ...
  "source_refs": ["core/docs/source.md"],
  "status": "current"
}}
```
<!-- /project-data -->
""",
    ...
    def test_document_refs_include_markdown_inside_untracked_nested_directory(self) -> None:
        with self._root() as raw_root:
            ...
            self.assertIn(
                "extension/docs/domain/youtube/contract.md",
                maintenance.document_refs(),
            )
    ...
    def test_scan_detects_drift_without_changing_lifecycle(self) -> None:
        with self._root() as raw_root:
            ...
            (Path(raw_root) / "core" / "docs" / "source.md").write_text(
                "# 출처\n\nversion two\n", encoding="utf-8"
            )
    ...
    def test_exact_document_replacement_suppresses_only_exclusively_migrated_legacy(
        ...
        with self._root() as raw_root:
            ...
            (root / "core" / "docs" / "source.md").write_text(
                "# 출처\n\nversion two\n",
                encoding="utf-8",
            )
    ...
    def test_unknown_terminal_wrong_type_and_duplicate_replacements_fail_closed(
        ...
        with self._root() as raw_root:
            ...
            self._write_knowledge_replacement(
                root,
                claim,
                target="core/docs/canonical-second.md",
    ...
    def test_scan_validates_canonical_failures_without_projection_records(self) -> None:
        with self._root() as raw_root:
            maintenance, _, _ = self._fixture(raw_root)
            failures = Path(raw_root) / "core" / "failures"
            failures.mkdir(parents=True)
            target = failures / "case.md"
            target.write_text(FAILURE_TEXT, encoding="utf-8")
            before_records = {
                path.name for path in (Path(raw_root) / "extension" / "data" / "records").glob("*.json")
            }
            ...
            self.assertEqual(failed["drift"][0]["target_id"], "core/failures/case.md")

    def test_legacy_failure_coexists_without_default_search_or_drift_duplication(self) -> None:
        with self._root() as raw_root:
            maintenance, _, _ = self._fixture(raw_root)
            root = Path(raw_root)
            failures = root / "core" / "failures"
            failures.mkdir(parents=True)
            target = failures / "case.md"
            target.write_text(FAILURE_TEXT, encoding="utf-8")
            knowledge = KnowledgeService(
            ...
            before_records = {
                path.name for path in (root / "extension" / "data" / "records").glob("*.json")
            }
            before_events = {
                path.name: path.read_bytes()
                for path in (root / "extension" / "data" / "events").glob("*.jsonl")
            }
    ...
    def test_verify_passes_and_broken_link_fails_explicitly(self) -> None:
        with self._root() as raw_root:
            ...
            source_path = Path(raw_root) / "core" / "docs" / "source.md"
            source_path.write_text(
                source_path.read_text(encoding="utf-8") + "[끊김](missing.md)\n",
                encoding="utf-8",
            )
    ...
    def test_verify_checks_python_in_separate_domain_package(self) -> None:
        with self._root() as raw_root:
            ...
            self.assertTrue(
                any(
                    error.startswith("python:extension/src/youtube_domain/broken.py:")
                    for error in failed["errors"]
                )
            )

    def test_verify_blocks_core_changes_unless_exact_approval_gate_is_used(self) -> None:
        with self._root() as raw_root:
            maintenance, _, _ = self._fixture(raw_root)
            added = Path(raw_root) / "core" / "docs" / "approved-change.md"
            added.write_text("# 승인 대상\n", encoding="utf-8")
            blocked = maintenance.verify()
            self.assertFalse(blocked["ok"])
            self.assertIn(
                "core_change_requires_user_approval:core/docs/approved-change.md",
                blocked["errors"],
            )
    ...
    def test_cost_report_exposes_documents_records_events_and_runtime(self) -> None:
        with self._root() as raw_root:
            ...
            self.assertGreaterEqual(report["documents"]["documents"], 3)
    ...
    def test_cli_verify_returns_stable_failure_when_core_change_is_unapproved(self) -> None:
        with self._root() as raw_root:
            self._fixture(raw_root)
            blocked = Path(raw_root) / "core" / "docs" / "blocked-change.md"
            blocked.write_text("# 승인되지 않은 변경\n", encoding="utf-8")
            env = {
                **os.environ,
            ...
            self.assertEqual(error["kind"], "core_change_required")
