...
class SourceRecordTests(unittest.TestCase):
    ...
    def test_source_schema_matches_runtime_contract(self) -> None:
        schema = json.loads((ROOT / "core" / "schemas" / "source-payload-v1.schema.json").read_text(encoding="utf-8"))
    ...
    def test_local_source_create_read_list_and_integrity(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05a-local-") as raw_root:
            ...
            (root / "core" / "docs" / "source.md").write_text("검증된 출처\n", encoding="utf-8")
            service = self.make_service(root)
            created = service.create_source(
                source_kind="local_document",
                locator="core/docs/source.md",
    ...
    def test_local_source_change_and_protected_path_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05a-drift-") as raw_root:
            root = Path(raw_root)
            (root / "core" / "docs").mkdir(parents=True)
            source = root / "core" / "docs" / "source.md"
...
class KnowledgeRecordTests(unittest.TestCase):
    def source(self, service: KnowledgeService, root: Path) -> dict[str, object]:
        (root / "core" / "docs").mkdir(parents=True, exist_ok=True)
        (root / "core" / "docs" / "source.md").write_text("단일 소유자 계약\n", encoding="utf-8")
    ...
    def test_knowledge_schema_matches_runtime_contract(self) -> None:
        schema = json.loads(
            (ROOT / "core" / "schemas" / "knowledge-payload-v1.schema.json").read_text(encoding="utf-8")
        )
    ...
    def test_source_drift_preserves_historical_knowledge_but_fails_explicit_verify(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05b-drift-") as raw_root:
            ...
            (root / "core" / "docs" / "source.md").write_text("변경됨\n", encoding="utf-8")
...
class DecisionRecordTests(unittest.TestCase):
    ...
    def test_decision_schema_matches_runtime_contract(self) -> None:
        schema = json.loads(
            (ROOT / "core" / "schemas" / "decision-payload-v1.schema.json").read_text(encoding="utf-8")
        )
...
class FailureKnowledgeTests(unittest.TestCase):
    def test_legacy_failure_schema_matches_runtime_contract(self) -> None:
        schema = json.loads(
            (ROOT / "core" / "schemas" / "failure-knowledge-payload-v1.schema.json").read_text(encoding="utf-8")
        )
    ...
    def test_failure_document_validates_and_lists_without_structured_files(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05d-failure-") as raw_root:
            root = Path(raw_root)
            (root / "core" / "failures").mkdir(parents=True)
            (root / "core" / "failures" / "neutral.md").write_text(failure_document(), encoding="utf-8")
            service = KnowledgeService(
                root,
                _write_capability=TEST_WRITE_CAPABILITY,
            )
            service.initialize()
            before = list((root / "extension" / "data" / "records").glob("*.json"))
            validated = service.import_failure_knowledge(
                "core/failures/neutral.md", projected_by="agent:test"
            )
            ...
            self.assertEqual(
                ...
                "core/failures/neutral.md",
            )
            self.assertEqual(list((root / "extension" / "data" / "records").glob("*.json")), before)
    ...
    def test_failure_document_rejects_unresolved_missing_section_and_invalid_projection_ids(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05d-invalid-") as raw_root:
            ...
            with self.assertRaises(KnowledgeRecordError):
                service.import_failure_knowledge("core/failures/unresolved.md", projected_by="agent:test")
            (failures / "missing.md").write_text(
                failure_document(include_prevention=False), encoding="utf-8"
            )
            with self.assertRaises(KnowledgeRecordError):
                service.import_failure_knowledge("core/failures/missing.md", projected_by="agent:test")
            target = failures / "prevalidation.md"
            target.write_text(failure_document(), encoding="utf-8")
            before = list((root / "extension" / "data" / "records").glob("*.json"))
            with self.assertRaises(KnowledgeRecordError):
                service.import_failure_knowledge(
                    "core/failures/prevalidation.md", projected_by="", record_id="not-a-uuid"
                )
            self.assertEqual(list((root / "extension" / "data" / "records").glob("*.json")), before)
            with self.assertRaises(KnowledgeRecordError):
                service.import_failure_knowledge(
                    "core/failures/prevalidation.md",
...
class SourceCliTests(unittest.TestCase):
    ...
    def test_source_cli_create_show_list_verify(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05a-cli-") as raw_root:
            root = Path(raw_root)
            (root / "core" / "docs").mkdir(parents=True)
            (root / "core" / "docs" / "source.md").write_text("CLI 출처\n", encoding="utf-8")
    ...
    def test_knowledge_cli_create_show_and_list(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05b-cli-") as raw_root:
            root = Path(raw_root)
            (root / "core" / "docs").mkdir(parents=True)
            (root / "core" / "docs" / "source.md").write_text("CLI 지식 출처\n", encoding="utf-8")
            self.assertEqual(self.run_cli(root, "init").returncode, 0)
            source = self.run_cli(
                root, "source-create", "--kind", "local_document", "--locator", "core/docs/source.md",
    ...
    def test_failure_cli_validates_and_lists_without_record_creation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="stage05d-cli-") as raw_root:
            ...
            validated = self.run_cli(
                root, "failure-validate", "--doc", "core/failures/neutral.md"
            )
            self.assertEqual(validated.returncode, 0, validated.stderr)
            self.assertFalse(json.loads(validated.stdout)["result"]["stored"])
            listed = self.run_cli(root, "failure-list")
            self.assertEqual(json.loads(listed.stdout)["result"]["count"], 1)
            self.assertEqual(list((root / "extension" / "data" / "records").glob("*.json")), [])
