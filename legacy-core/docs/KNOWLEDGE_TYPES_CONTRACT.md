...
- 책임: `core/src/file_data/knowledge.py`가 의미 검증과 참조 무결성, `core/schemas/*-payload-v1.schema.json`이 저장 record 구조, `core/failures/*.md`가 실패 사례의 사람·기계 공통 정본을 소유한다.
- 상태: 활성 계약.
...
| transient failure view | `core/failures/*.md` 정본을 실행 시 구조적으로 읽은 비저장 view | 별도 정본, lifecycle 대상, 장기 저장 projection |
...
`core/failures/*.md`가 해결 실패 사례 본문과 현재 유효성의 유일한 정본이다. parser는 다음 필드를 실행 시 읽어 transient view를 만들며 파일로 저장하지 않는다.
...
검증은 `core/failures/README.md`를 제외하고 상태에 `해결`이 있으며 `증상`, `확인된 원인`, `해결과 검증`, `재사용 규칙` 절이 모두 있는 사례만 받는다. `해결과 검증` 절의 첫 목록 항목을 해결, 나머지를 검증 근거로 읽는다. strict UTF-8, NUL 0, SHA-256과 필수 의미를 매번 정본 bytes에서 계산한다.
...
복합 decision payload는 PowerShell에서 UTF-8 stdin과 `--payload-stdin`을 사용한다. 모든 record는 common-record v1 외피와 `extension/data/records/<id>.json` 단일 주소를 재사용한다.

## 구조 계약

- `core/schemas/source-payload-v1.schema.json`
- `core/schemas/knowledge-payload-v1.schema.json`
- `core/schemas/decision-payload-v1.schema.json`
- `core/schemas/failure-knowledge-payload-v1.schema.json` — legacy stored projection read compatibility
