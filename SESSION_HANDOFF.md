...
- 현재 작업: 재현 가능한 프로젝트 기반 M1 Fresh Clone·Core 순수성 (exact 경계 승인 대기)
- 상태: M0-S1~S3와 M0-X1~X4 통과, M0 커밋 전환 후 M1 exact Core 범위 산출 단계
- 활성 전체 설계: `extension/work/PROJECT_FOUNDATION_DESIGN.md`
- 활성 단계 설계: `extension/work/project-foundation/M1_FRESH_CLONE_AND_CORE_PURITY.md`
...
- 현재 branch는 `main`, HEAD는 `07a26a3`이며 범용 파일 추출·정리 규칙은 Core owner로 교정됐다.
...
- M1 구현 전 exact `core/**` 경로·이유에 대한 현재 대화의 사용자 승인이 필요하다.
...
| `extension/work/project-foundation/M1_FRESH_CLONE_AND_CORE_PURITY.md` | active phase-design·draft | M1 exact 범위·slice·entry/exit/transition gate |
...
1. M1-S1에서 root·Core·Extension exact 후보 경로와 이유를 확정하고 Core 승인 경계를 판정한다.

> M0은 통과했다. M1 Core 변경은 exact 경로·이유 승인 전 실행하지 않으며, dependency 설치·CI·보호 데이터 접근도 별도 경계를 유지한다.
