...
- 상태: 7C source·knowledge·decision·승인 lifecycle 로컬 완료. 사용자 단계 확인 대기.
...
- `legacy-absorption-7c-knowledge-lifecycle`: source·knowledge·decision 의미 검증과 event 정본 기반 승인 lifecycle을 새 L7 저장 기반 위에 일반화하고 관련 계약·schema·회귀를 완료했다.

## 직전 게이트

- `pass`: 지식·lifecycle 회귀, Core gate, Maintainer 소비 gate와 Core 후보 commit clean-clone 검증을 통과했다. 선택 기능은 아직 등록되지 않아 의도대로 `not_applicable`이다.

## 승인 상태

- 7C의 source·knowledge·decision·lifecycle 구현, 계약·schema·회귀와 Core·부모 로컬 커밋이 승인됐다.
- 후속 Evidence Context·작업 상태 Runtime·공개 CLI·기존 `file_data` 의존 전환, 정책·규칙의 외부 모델 서비스 전송과 모든 push·원격 조작은 승인되지 않았다.
...
- Evidence Context와 작업 상태 Runtime은 아직 `legacy-core`에만 있다.
...
1. 사용자가 7C 완료를 확인하면 7D에서 문서·`data_key` 직접 선택과 결정론적 Evidence Context package를 새 L7 지식 기반 위에 흡수한다.
