...
- 상태: `P0`와 단계 0 완료. 단계 1 진입 준비.
- 관련 권위: `core/PROJECT_RULES.md`, `PROJECT_RULES.md`.
- 활성 전체 설계: `AGENT_CORE_REMAINING_WORK_PLAN.md`.
- 활성 단계 설계: `AGENT_CORE_REMAINING_WORK_PLAN.md` §7 `단계 1 — L7 격리와 Python 3.10 복구`.
- handoff mode: `same-workspace`.
- uncommitted dependency: Core `P0` commit은 `9751d7f`이며, 부모의 Core gitlink·통합 설계·이 핸드오프 갱신은 현재 단계 커밋 대상이다. 기존 검증 부산물은 단계 4의 오염 탐지·정리가 소유한다.

## 현재 목표

단계 1에서 `R1` 선택 기능 격리와 `R2` Python 3.10 호환성을 구현한다. L7 완전 설치·완전 부재·부분 결손을 구분하고 실제 최소 Runtime과 테스트 수집 누락을 정확히 판정한다.
...
- 완료: `R0` 규칙 거버넌스 개선, 통합 설계의 검증 의무 재대조, 단계 0 기준점·복구 bundle 검증.
...
1. 단계 1의 L7·Python 호환 구현과 테스트 표면을 현재 파일에서 조사한다.
2. 정확한 변경 경로를 확정하고 `R1`, `R2`의 직접 관련 구현과 검사만 수정한다.
3. 단계 1 관련 검사를 통과시킨 뒤 Core·부모 후보 commit을 만들고 단계 2로 전환한다.

## 다음 세션 시작 prompt

Core 정책, 소비 정책, 이 문서를 순서대로 읽는다. 보호 경로에 접근하지 않는다. `P0`와 단계 0은 완료됐고 활성 단계는 통합 설계 §7의 단계 1이다. 현재 파일에서 L7·Python 3.10 표면을 조사하고 관련 검사만 선택한다. 단계별 commit은 승인됐지만 push는 별도 승인 전까지 하지 않는다.
