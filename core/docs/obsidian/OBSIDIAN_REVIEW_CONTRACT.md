...
| 규칙 탐색 | `PROJECT_RULES.md`의 조건부 route | 별도 규칙 index 없이 필요한 규칙만 읽음 |
...
3. 정책과 작업 규칙은 [PROJECT_RULES](../../../PROJECT_RULES.md), 플랫폼 진입점은 [AGENTS](../../../AGENTS.md)
4. 완료 상세는 Git 이력
...
- `PROJECT_RULES.md`는 활성 `core/rules/*.md` 각각을 정확히 한 번 Markdown link로 연결해야 하며, `AGENTS.md`는 core rule에 직접 연결하지 않고 활성 규칙 문서를 고아 node로 남기지 않는다.
