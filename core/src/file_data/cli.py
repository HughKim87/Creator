...
from .maintenance import CoreChangeRequiredError, MaintenanceError, MaintenanceService
...
def _parser() -> RecordArgumentParser:
    ...
    commands.add_parser(
        "init",
        help="create approved extension/data/records and extension/data/events directories",
    )
    ...
    maintenance_verify = commands.add_parser(
        "maintenance-verify", help="fail-closed project structure verification"
    )
    maintenance_verify.add_argument(
        "--allow-core-changes",
        action="store_true",
        help="use only after explicit user approval for the exact core change scope",
    )
    ...
    *,
    ...
    if namespace.command.startswith("maintenance-"):
        ...
        if namespace.command == "maintenance-verify":
            result = maintenance.verify(
                allow_core_changes=namespace.allow_core_changes,
            )
            if not result["ok"]:
                core_errors = [
                    error
                    for error in result["errors"]
                    if error.startswith("core_change_requires_user_approval:")
                ]
                if core_errors:
                    raise CoreChangeRequiredError("; ".join(core_errors))
                raise MaintenanceError(
                    "maintenance verification failed: " + "; ".join(result["errors"])
                )
            return result
    ...
    if namespace.command.startswith("decision-"):
        ...
        if namespace.command == "decision-create":
            ...
            return {
                "record": record,
                "path": f"extension/data/records/{record['id']}.json",
            }
    ...
    if namespace.command.startswith("work-"):
        ...
        if namespace.command == "work-create":
            ...
            return {
                "state": state,
                "path": f"extension/data/records/{state['id']}.json",
            }
