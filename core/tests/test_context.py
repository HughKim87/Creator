...
class ContextServiceTests(unittest.TestCase):
    def _fixture(self, raw_root: str) -> tuple[ContextService, dict, dict]:
        ...
        (root / "core" / "docs" / "route.md").write_text("# 직접 라우팅\n\n필수 계약만 읽는다.\n", encoding="utf-8")
    ...
    def test_direct_document_and_current_record_build_deterministic_package(self) -> None:
        with self._root() as raw_root:
            ...
            request = {
                "purpose": "직접 선택 평가",
                "documents": [
                    {"ref": "core/docs/route.md", "reason": "필수 계약"},
                    {"ref": "core/docs/route.md", "reason": "중복 직접 경로"},
                ],
                "records": [
    ...
    def test_failure_search_uses_canonical_document_without_record_fanout(self) -> None:
        with self._root() as raw_root:
            ...
            self.assertEqual([item["ref"] for item in matches], ["core/failures/context-case.md"])
            package = context.build_package(
                {
                    "purpose": "실패 재사용",
            ...
            self.assertEqual(
                ...
                [("document", "core/failures/context-case.md")],
            )
            target.write_text(
    ...
    def test_required_context_over_limit_fails_without_truncation(self) -> None:
        with self._root() as raw_root:
            ...
            with self.assertRaises(ContextLimitError):
                context.build_package({
                    "purpose": "크기 제한",
                    "documents": [{"ref": "core/docs/route.md", "reason": "필수 계약"}],
                    "char_limit": 1,
                })
    ...
    def test_optional_search_candidate_is_excluded_at_size_limit(self) -> None:
        with self._root() as raw_root:
            context, _, _ = self._fixture(raw_root)
            document_chars = context.measure_documents(["core/docs/route.md"])["characters"]
            package = context.build_package({
                "purpose": "선택 후보 크기 제한",
                "documents": [{"ref": "core/docs/route.md", "reason": "필수 계약"}],
                "search": "전체 읽기",
    ...
    def test_baseline_counts_exact_unique_documents(self) -> None:
        with self._root() as raw_root:
            context, _, _ = self._fixture(raw_root)
            measured = context.measure_documents(["core/docs/route.md", "core/docs/route.md"])
            raw = (Path(raw_root) / "core" / "docs" / "route.md").read_bytes()
    ...
    def test_actual_project_defaults_to_document_records_without_runtime_data(
        ...
        package = service.build_package(
            {
                ...
                "documents": [
                    {
                        "ref": "core/docs/CONTEXT_PACKAGE_CONTRACT.md",
                        "reason": "전체 계약 직접 선택",
                    }
                ],
                "search": "결정론적",
        ...
        self.assertEqual(
            [
                ...
                if item["ref"] == "core/docs/CONTEXT_PACKAGE_CONTRACT.md"
            ],
            ["core/docs/CONTEXT_PACKAGE_CONTRACT.md"],
        )
