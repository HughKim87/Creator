...
class WorkStateServiceTests(unittest.TestCase):
    ...
    def test_work_state_schema_matches_projected_payload(self) -> None:
        schema = json.loads((ROOT / "schemas" / "work-state-payload-v1.schema.json").read_text(encoding="utf-8"))
        expected = {
    ...
    def test_request_and_event_schemas_match_runtime_fields(self) -> None:
        request_schema = json.loads(
            (ROOT / "schemas" / "work-request-payload-v1.schema.json").read_text(encoding="utf-8")
        )
        event_schema = json.loads(
            (ROOT / "schemas" / "work-event-payload-v1.schema.json").read_text(encoding="utf-8")
        )
