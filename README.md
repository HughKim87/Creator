...
이 프로젝트는 특정 영상 제작 도구를 자동화하기 위한 프로젝트가 아닙니다.

사람이 매번 모든 맥락을 다시 설명하고 반복 작업을 직접 수행하지 않아도, AI가 사용자의 의도와 판단 기준, 진행 상태를 이어받아 실제 업무를 계속 수행할 수 있는 환경을 만드는 것이 목적입니다. 자동화의 범위는 넓히되, 최종 결정권과 민감 정보, 외부 행동의 권한은 사용자에게 남겨 둡니다.

이를 위해 필요한 정보만 선택하고, 적절한 도구를 실행하고, 결과를 검증하고, 중단된 작업을 다시 이어 가는 공통 구조를 구현했습니다. 현재는 YouTube·영상 제작과 게임 리서치가 대표적인 확장 영역입니다.
...
```mermaid
flowchart TD
    U["사용자 요청"] --> R["시작 경로 확인<br/>규칙 · 권한 · 현재 상태"]
...
1. `AGENTS.md`가 프로젝트의 시작점 역할을 합니다.
2. `PROJECT_RULES.md`와 현재 상태 문서가 권한, 보호 데이터, 작업 범위를 결정합니다.
...
| 파일 | 역할 |
|---|---|
| `AGENTS.md` | 새 에이전트가 가장 먼저 읽는 진입점 |
| `PROJECT_RULES.md` | 권한, 보호 데이터, 변경 범위, 조건부 규칙의 최상위 정책 |
...
| 도구·형식 | 사용하는 이유 |
|---|---|
| Codex Agent Skills | 복잡한 작업 절차를 재사용 가능한 capability 단위로 분리하고 요청에 맞춰 자동 선택하기 위해 사용합니다. |
...
이 README는 사람을 위한 개요입니다. 에이전트의 실제 시작 경로는 `AGENTS.md` → `PROJECT_RULES.md` → `SESSION_HANDOFF.md`이며, 작업별로 필요한 규칙과 계약만 추가로 읽습니다.
