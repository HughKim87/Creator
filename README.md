...
현재 영상 제작 워크플로와 그 과정에서 축적되는 데이터는 **AI Agent의 자율성과 협업 방식을 실제 업무에서 검증하기 위한 테스트** 데이터입니다.

또한 이 프로젝트는 **하나의 AI Agent에 종속된** 자동화 프로젝트도 아닙니다.
실제로 현재 Claude와 Codex가 동일한 프로젝트에서 함께 작업하고 있으며, 언제든지 Agent 를 교체 가능한 구조를 목표로 구성하고 있습니다.

이 프로젝트의 목적은 사람이 매번 모든 맥락을 다시 설명하지 않아도, 서로 다른 Agent가 **동일한 규칙·상태·지식·작업 근거를 공유하며 실제 업무를 이어서 수행할 수 있는 환경**을 만드는 것입니다. 어떤 Agent가 작업을 시작하거나 넘겨받더라도 사용자의 의도와 판단 기준이 유지됩니다.

핵심은 프로젝트 상태를 특정 Agent의 대화 기억에 가두지 않는 것입니다. 이를 위해 필요한 정보만 선택하고, 적절한 도구를 실행하고, 결과를 검증하고, 다른 Agent도 중단 지점부터 재개할 수 있는 공통 구조를 구현했습니다.


## 이 구조에서 중요하게 보는 것

- 에이전트의 자율성은 사용자가 승인한 목표와 경계 안에서만 확장합니다.
- 특정 Agent 에 종속하지 않으며 현재 Claude와 Codex가 같은 프로젝트 상태와 작업 기준을 공유합니다.
...
- **06-26** 프로젝트 목표와 자동화 범위를 정리하고 초기 작업 자료를 축적하며 프로젝트를 시작했다.
- **07-02** 기존 자료를 현 폴더로 이관하고 Git을 시작하면서 작업 규칙 원본과 에이전트 진입점을 통일했다.
- **07-09** 작업 규칙과 실행 부트스트랩을 분리해 초기 진입 구조를 단순화했다.
- **07-14** 규칙 준수와 작업 상태를 공통으로 통제하는 도구와 테스트를 구축했다.
- **07-16 — 1차 리팩토링** 작업을 단계별로 실행·검증하는 구조와 자동 품질 검사를 도입했다.
- **07-17** 도메인 모델과 SQLite 기반 상태 저장소를 구축해 작업 상태의 정본을 만들었다.
- **07-18 — 2차 리팩토링** 루트 문서와 디렉터리를 정리하고 프로젝트 기반을 다시 구축하기 시작했다.
- **07-20** 기존 구현에 의존하지 않고 기본 규칙과 프로젝트 구조를 빈 상태에서 재수립했다.
- **07-21** 컨텍스트 카탈로그·지식 저장소·검색·작업 기록 시스템을 하나의 체계로 통합했다.
- **07-22 — 3차 리팩토링** 기존 구축 결과를 실사용 관점에서 역추론해 전체 구조를 다시 설계했다.
- **07-24 — 4차 리팩토링** 추적 중이던 과거 문서와 백업을 대규모 삭제하고 공통 기반과 도메인 기능을 분리했다.
- **07-28** 요청별 규칙 선택 경로와 작업 상태를 소유하는 문서를 하나로 정리했다.
- **07-29 — 5차 리팩토링** 설계·실행 계약, 변경 무효화, 단일 품질 게이트와 도메인 무관 기반 검증을 확립했다.
- **07-31 — 6차 리팩토링** 전체 파일을 재분류하고 규칙·지식·산출물·실행 환경의 보존 및 생명주기를 통합 정리했다.
- **08-01** 43개 작업 커밋을 보존하면서 보호 자료와 대용량 객체를 제거하고 정본 원격 저장소로 이력을 이전했다.
