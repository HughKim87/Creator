...
- 읽는 시점: 공통 기록을 읽거나 쓸 때, Stage 04 이후 유형을 연결할 때, I/O 오류를 진단할 때.
- 책임: `src/file_data/store.py`가 라이브러리 동작, `src/file_data/cli.py`가 명령·종료 상태를 소유한다.
- 상태: Stage 03 활성 계약.
...
직접 `open`, 임의 경로 쓰기, 내부 임시 파일 조작은 승인된 사용 경로가 아니다. `init`은 존재하는 프로젝트 루트 아래에 `data/records`와 `data/events`만 만든다.
...
- `data/events/<approved-stream>.jsonl`에 common-record v1 객체를 한 줄씩 저장한다.
...
- Stage 04는 승인 유형 집합에 작업 기록·현재 상태 유형을 추가하되 I/O 구현을 우회하지 않는다.
- Stage 05 이후 지식 의미·품질·수명주기는 `payload`와 승인 유형 계층이 소유한다.
- Stage 08 전에는 stale lock 자동 복구나 대규모 stream 최적화를 추가하지 않는다.
