...
- Context operations use the structured request → resolver → work context/write contract → transactional writer path in `tools/context/context_system.py`. The R-3 writer supports registered text, code, config, binary, move, and delete targets, verifies before-file and before-unit hashes, restores touched paths on partial failure, and updates catalog, unit, and event evidence. Knowledge candidates, reviews, revisions, and supersession use the dedicated lifecycle operation; source hash checks create review transitions. Decision and case lifecycle writers remain unimplemented and must not be claimed.
