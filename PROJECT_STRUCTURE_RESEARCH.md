...
| 기존 기본 시작 문서 | 122줄 |
| 전체 규칙까지 읽는 작업 | 283줄 |
| `backup/inputs` | 4개, 3.539GiB |
| `backup/outputs` | 41개, 0.408GiB |
...
현재 백업 데이터가 약 3.95GiB이므로 코드 저장소와 영상 데이터의 물리적 경계는
필수다. 다만 다중 PC 동기화나 재현 요구가 아직 정의되지 않았으므로 0단계에서
Git LFS나 DVC를 설치하지 않는다.
...
`PROJECT_BOOTSTRAP.md`, `PROJECT_RULES.md`, `SESSION_HANDOFF.md`를 각각 운영 규칙으로
다시 만들지 않는다. 루트 `AGENTS.md`와 `docs/INDEX.md`의 두 단계로 진입을 끝낸다.
현재 작성하는 `SESSION_HANDOFF.md`는 마이그레이션 기간의 재개 상태 정본이며, 향후
상태 엔진이 도입되면 생성 뷰로 대체할 수 있다.
