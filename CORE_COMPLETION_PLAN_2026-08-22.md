...
- L7 선택 기능이 없어도 필수 Core가 성립한다.
- 선언된 최소 Python 버전에서 Core가 동작한다.
- Core의 선택 기능과 특정 Consumer의 필수 의존성을 구분한다.
- 사용자 승인 조건은 정본 한 곳에서만 정의된다.
- 로컬 사본 검증과 실제 원격 submodule 검증을 구분한다.
- 검증 실행이 작업 트리를 오염시키지 않는다.
...
다음 조건을 모두 만족하면 추가 개선 목록을 만들지 않고 작업을 종료한다.
...
이 종료 규칙을 충족한 뒤에는 새로운 기능 요청이나 새로운 실패 증거가 없는 한 추가 개선작업을 제안하지 않는다.
