...
- 책임: 작업 에이전트가 이 문서의 단계·게이트·범위만 구현하고, 사용자가 정책 의미·Core 변경·commit·외부 쓰기·원격 게시를 승인한다.
- 상태: 원격 사용 검증 완료. 계획 작업 완료.
...
구조 검사의 통과는 최소 Python, 전체 회귀, 실제 원격 clone을 자동으로 증명하지 않는다.
...
- clone 임시 루트에서 Node 실행 가능성을 먼저 확인하고, 환경 공통 실패면 복제 작업 전에 중단한다.
- bootstrap이 실패한 복제본에서는 의존하는 전체 verify를 실행하지 않는다.
...
`P0`, 단계 0~4와 §11 원격 사용 검증으로 이 설계의 작업은 완료됐다. 실제 프로젝트 사용은 별도 검증 단계로 계획하지 않으며, 사용 중 발견된 문제는 새 Maintainer 요청으로 다룬다.

main 반영·태그·릴리스는 각각 별도 지시가 있을 때 진행한다.
