...
`P0`와 단계 0~4의 로컬 구현 및 §11의 원격 사용 검증은 완료됐다. 다음 실행 대상은 §12의 실제 Host 검증이다.

1. Core 후보 `e1c7c63`과 Maintainer 후보 `5255009`의 원격 도달성을 유지한다.
2. 첫 실제 Host 저장소와 그 프로젝트가 소유할 정책·상태·보호 경계를 확정한다.
3. Host 읽기 전용 권한으로 Core를 연결하고 Codex·Claude 재개 검증을 수행한다.

main 반영·태그·릴리스는 실제 Host 검증 결과 보고 뒤 결정한다.
