...
`P0`와 단계 0~4의 로컬 구현은 완료됐다. 다음 실행 단계는 §11이며 아래 조건을 모두 만족할 때만 진입한다.

1. 사용자가 Core와 Maintainer push를 별도로 승인한다.
2. 사용할 원격 ref와 인증 방법이 확인된다.
3. 보호 경로, 실제 Host 적용, 태그·릴리스는 승인 범위에 포함되지 않음을 유지한다.

승인이 없으면 현재 로컬 완료 상태를 유지하고 추가 작업을 시작하지 않는다.
