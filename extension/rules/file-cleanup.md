...
- 행동:
  1. `file-extraction` 규칙으로 문서의 성공한 절차·검증 기준·재사용 가능한 실패·현재 상태·사용자 승인·source lineage를 분류한다.
  2. 성공 지식은 특정 작업명·파일명·일회성 수치를 제거하고 `조건 / 행동 / 예외 / 검증`으로 압축해 가장 좁은 기존 active owner에 반영한다. 반복 근거가 부족하면 candidate owner에 둔다.
